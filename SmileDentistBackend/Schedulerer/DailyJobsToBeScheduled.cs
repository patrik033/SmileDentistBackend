...
namespace SmileDentistBackend.Schedulerer
{
    ...
    public class DailyJobsToBeScheduled : IJob
    {
        ...
        public DailyJobsToBeScheduled(ILogger<DailyJobsToBeScheduled> logger, IServiceProvider serviceProvider, ISendGridEmailBookings sender)
        {
            _logger = logger;
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                {
                    var some = await _sender.SendAsync("patrik.odh@gmail.com", "patrik.odh@gmail.com", "Smile Dentist Booking", $"Hello {item.Name} you have a reservation for dental care at: {item.ScheduledTime}", item.ScheduledTime, item.Name);
